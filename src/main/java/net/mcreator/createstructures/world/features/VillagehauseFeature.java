...
public class VillagehauseFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("create_structures", "vallage_hause"));
		if (template == null)
			return false;
...
//		System.out.println(template.getSize());
		...
		if ((context.random().nextInt(1000000) + 1) <= 100000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (!Additional_genProcedure.execute(world, x, y, z, struct_x, struct_y, struct_z, rotate_angle))
					continue;
